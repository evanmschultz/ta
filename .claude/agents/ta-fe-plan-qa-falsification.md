...
- **Atomic violations**: any droplet over the **2-block budget** that should be converted to a `cascade.planner` sub-plan? A 3-block "droplet" is the anti-pattern — the planner must emit a sub-planner instead. Hunt droplets doing 2+ distinct things.
