...
- **Atomic decomposition (MEASURE, don't trust the label)**: for every leaf `cascade.droplet`, COUNT the distinct new/changed production symbols/components named (tests excluded) and estimate diff LOC. Confirm each is **1-2 blocks: ≤80 LOC incl. tests, ≤3 files, ≤2 distinct production symbols/components** AND has declared `paths`. Any droplet at **≥3 distinct symbols/components / >80 LOC / >3 files** is a methodology violation — FAIL with the directive to convert it to a `cascade.planner` sub-plan. Do NOT accept the planner's "1-2 blocks" assertion without counting; on a plan amendment, re-verify EVERY droplet's budget, not just the amended one. STATE each droplet's prod-LOC and test-LOC SEPARATELY in your Coverage Check (e.g. `d3_writer: ~90 prod + ~120 test = 210 ✗ SPLIT`) so the estimate is auditable, and treat "one coherent concern" / "a single cohesive function" as NOT an exception to the budget.
