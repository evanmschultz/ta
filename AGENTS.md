...
- `mage test` / `mage check` / `mage cover` route through `laslig/gotestout` which auto-detects TTY status — agents and CI pipes get plain text, humans get a styled summary. No env-var prefix needed.
