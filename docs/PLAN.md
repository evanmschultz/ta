...
- **Cross-db `paths` overlap detection** (locked 2026-04-25). Two dbs cannot share a `paths` entry. Mount-string equality is the test, regardless of format — `paths = ["plans"]` toml and `paths = ["plans"]` md both reject because the mount strings collide and the address grammar (`<file-relpath>.<type>.<id-tail>`, no db prefix) cannot disambiguate them. Within a single format, mount entries that ext-normalize to the same form (e.g. `["plans"]` and `["plans.toml"]` for `format = "toml"`) also overlap. Collection mounts (trailing `/`) match descendants only; they do NOT shadow a sibling file with the same prefix (`["docs/"]` md and `["docs.md"]` md are accepted as disjoint).
